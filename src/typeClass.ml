...
module Exceptions = struct 
  ...
  (** erreur d'Héritage *)
  exception Her of pos * sident * string
...
module  CheckInstr = struct 
 ...
 (** calcule si une classe c1 hérite d'une autre classe c2 *)
  ...
  let isSubProf classes p1 p2 = 
    try 
      List.for_all2 (fun t1 t2 -> isSubType classes t1 t2)
    with Failure _ -> false

  (** compare deux profils, relation d'ordre supposée totale *)
  let compProfs classes p1 p2 = 
  ...
  type leftValue = { lv : Sast.vars; lt:  Sast.types }
       
  (** Type les varleurs gauches *)
  let rec typLeft classes c env = function
    |Var(id) -> 
      let t =
	try 
	  Cmap.find id env 
	with Not_found ->
      ...
      in
      { lv = SVar({id_id = id; id_typ = t}) ; st = t }
    | Attr(e,id) ->
	...
	match exp.st with
	  | SC f ->(
	    try 
	      Cmap.find f classes 
	    with Not_found -> (raise (Undefined(exp.p,f))))
	...
	with Not_found -> (raise (Missing(p,cl.oclass_name,id)))
      in
      { lv = SAttr(exp,{id_id = id; id_typ = types_to_Sast attr.v_type}); st =  types_to_Sast attr.v_type }
  ...
  and typExpr classes c env = function
    ...
    | Getval(p,left) ->
      let  left = typLeft classes c env left in
      { sv = left.lv;  st = left.lt ; sp = p }
    | Assign(p,left,e) ->
      let left = typLeft classes c env left in
      let exp = typExpr classes c env e in
	...
	{ sp = p; st = left.lt; sv = left.lv}
    | Pref(p,op,left) ->
      let left = typLeft classes c env left in
      if left.lt != SInt then
	(raise (WrongType (p,left.lt, Some (SInt))))
      else
	{sv = SPref(p,op,left.lv); st = SInt; sp = p }
    | Post(p,op,left) -> 
      let left = typLeft classes c env left in
      if left.lt != SInt then
	(raise (WrongType (p,left.lt, Some (SInt))))
      else
	{sv = SPost(p,op,left.lv); st = SInt; sp = p}
    | UMinus (p,e) ->
	...
	match op with 
	  ...
	  | Leq | Geq | Lt | Gt -> 
	    if e1.st = SInt then
		  ...
		  (raise (WrongType(e2.sp,e2.sp,Some(SInt)))))
	    else
	  ...
	  | And | Or ->
	    if e1.st = SBool then
		  ...
		  (raise (WrongType(e2.sp,e2.sp,Some(SBool)))))
	    else
	...
	| None ->  
	  ({ sp = p; st = SC c.oclass_name; sv = SGetval(SVar({id_id = "this"; id_tp = SC  c.oclass_name}))}),c
      in
	...
	(
	  ...
	  let mList = List.stable_sort compProfs mList in
	  let meth = 
	    try
	      List.hd mList 
	...
	with Not_found -> (raise (Undefined(p,n)))
      ...
      let li = List.fold_left (fun acclist elt ->
	if (isSubProf classes args (List.map (fun v -> types_to_Sast v.v_type) elt.ocall_args)) then
	  (elt::acclist)
	else
	  acclist)
	[] cl.oclass_consts  in
	...
	match e.st with 
	  | SC _ | STypeNull ->  
	    if ((isSubType classes t e.st) && (isSubType classes e.st t) ) then
	      { sv = SInstanceof(t,e); sp = p; st = SBool }
	    else
  ...
  let typInstr classes c env = function
    | Expr(e) ->
      let e = typExpr classes c env e in
      env,SExpr(e)
    | Decl(p,t,id,None) ->       
      ...
      (Cmap.add id.id_id id env),(SDecl(p,t,id,None))
    | Decl(p,t,id,Some e) ->
      if not (ClassAnalysis.isBF classes t) then (raise (WrongType(p,types_to_Sast t),None));
      let t = types_to_Sast t in
      let id = { id_id = id; id_typ = t } in
      ...
      ((Cmap.add id.id_id id env),(SDecl(p,t,id,Some e)))
    | If(e,i1,i2) ->
      let e = typExpr classes c env e in
      if not (e.st = SBool) then (raise (WrongType(e.sp,e.st,Some SBool)));
      let env1,i1 = typInstr classes c env i1 in
      let env2,i2 = match i2 with 
	| None -> None 
	| Some i -> Some (typInstr classes c env i)
      in
      (env,(SIf(e,i1,i2)))
    | For(e1,e2,e3,i) -> 
      let _,e1 = match e1 with
	| None -> None 
	| Some e ->  Some (typExpr classes c env e)
      in
      let _,e3 = match e3 with
	| None -> None 
	| Some e ->  Some (typExpr classes c env e)
      in
      let _,e2 = match e2 with 
	| None -> {sv = SBconst true ; sp = { file = "unknown"; line = 0; fChar = 0; lChar = 0}; st = SBool}
	| Some e2 -> (
	  let e2 = typExpr classes c env e in
	  if e2.st != SBool then (raise (WrongType(e.sp,e.st,Some SBool)));
	  e2)
      in
      let _,i = match i with
	| None -> None
	| Some f -> typInstr classes c env f 
      in
      (env,(SFor(e1,e2,e3,i)))
    | Block(li) -> 
    ...
    | Return(e) ->
      let e = match e with 
	| None -> None 
	| Some f -> typExpr classes c env f 
      in
      (env,(SReturn(e)))
